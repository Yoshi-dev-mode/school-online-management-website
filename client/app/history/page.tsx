...
export default function HistoryPage() {
  ...
  const { orders: localOrders } = context;
  ...
  // Flatten orders to table rows
  const allOrderItems = backendOrders.flatMap((order: any) =>
  ...
  return (
    <main>
      <div className="flex items-center mb-4">
        <Image src="/history.png" height={30} width={30} alt="history icon" />
          ...
          <tbody>
            {filteredHistory.map((item: any, i: number) => (
              <tr key={i} className="border-b text-gray-900">
                ...
                <td className="py-3 px-2">₱{(item.price * item.quantity).toFixed(2)}</td>
